# Authors and Citation

## Authors

- **Tae Hyun Lim**. Author, maintainer.
  [](https://orcid.org/0000-0002-0429-2683)

## Citation

Source:
[`inst/CITATION`](https://github.com/taehyun-lim/krpoltext/blob/v0.1.0/inst/CITATION)

Lim T (2025). “South Korean Election Campaign Booklet and Party
Statements Corpora.” *Scientific Data*, **12**, 1030.
[doi:10.1038/s41597-025-05220-4](https://doi.org/10.1038/s41597-025-05220-4).

    @Article{,
      title = {South Korean Election Campaign Booklet and Party Statements Corpora},
      author = {Tae Hyun Lim},
      journal = {Scientific Data},
      year = {2025},
      volume = {12},
      pages = {1030},
      doi = {10.1038/s41597-025-05220-4},
    }

Lim T (2024). “South Korean Election Campaign Booklet Corpus and Party
Statements Corpus.”
[doi:10.17605/OSF.IO/RCT9Y](https://doi.org/10.17605/OSF.IO/RCT9Y), Open
Science Framework, <https://osf.io/rct9y/>.

    @Misc{,
      title = {South Korean Election Campaign Booklet Corpus and Party Statements Corpus},
      author = {Tae Hyun Lim},
      year = {2024},
      doi = {10.17605/OSF.IO/RCT9Y},
      url = {https://osf.io/rct9y/},
      note = {Open Science Framework},
    }

Lim T (2025). *krpoltext: Korean Political Text Corpora for R*. R
package version 0.1.0, <https://github.com/taehyun-lim/krpoltext>.

    @Manual{,
      title = {krpoltext: Korean Political Text Corpora for R},
      author = {Tae Hyun Lim},
      year = {2025},
      url = {https://github.com/taehyun-lim/krpoltext},
      note = {R package version 0.1.0},
    }
