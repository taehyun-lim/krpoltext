...
artifact filenames. The enriched variant uses suffixed filenames and
adds conservative NEC linkage fields such as `huboid`, `sg_id`,
...
If GitHub Pages is temporarily returning `404` for one of the JSON
files, the same published artifacts are mirrored on the `gh-pages`
branch:
